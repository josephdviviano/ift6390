...
class regression_gradient:
    ...
    def train(self, X, y):
        ...
        self.n = np.shape(X)[0]
        self.d = np.shape(X)[1]
        self.w = np.random.uniform(low=-0.01, high=0.01, size=self.d + 1) # includes b at position 0
        ...
        for i in range(self.n_steps):
            ...
            gradient = (empirical_risk + regularization) #/ self.n

            ## stopping criteria
            #if np.sum(np.abs(gradient - old_grad)) < stopping_tolerance:
            #    break
...
train_X = iris[:, :-1]
train_Y = iris[:, -1]
mdl = regression_gradient(lamb=0.005, step_size=0.01, n_steps=10)
mdl.train(train_X, train_Y)
print(mdl.w)
...
mdl_2 = regression_gradient(lamb=62.5, step_size=step_size, n_steps=n_steps)
mdl_2.train(D[:,0], D[:,1])
axs[0][0].plot(x, mdl_2.predict(x), color='green', alpha=alpha)

# plot regression_gradient - lambda large
mdl_3 = regression_gradient(lamb=125, step_size=step_size, n_steps=n_steps)
mdl_3.train(D[:,0], D[:,1])
...
width = 1.0
axs[0][1].bar(lambs, avg_loss, width, color='b' )
